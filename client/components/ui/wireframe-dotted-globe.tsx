...
export default function RotatingEarth({ width = 800, height = 600, className = "" }: RotatingEarthProps) {
  ...
  useEffect(() => {
    ...
    // Get the computed primary color from the theme
    const computedStyle = getComputedStyle(document.documentElement)
    const brandColor = computedStyle.getPropertyValue("--brand-cyan").trim() || "#34a85a"
    ...
    const render = () => {
      ...
      // Globe background — transparent, just the outline
      context.beginPath()
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      const newRadius = Math.max(radius * 0.7, Math.min(radius * 2, projection.scale() * sf))
